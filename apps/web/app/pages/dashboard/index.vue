<template>
  <div>
    <!-- Page header -->
    <div class="mb-8">
      <h1 class="text-2xl font-bold text-neutral-900 dark:text-white">控制台</h1>
    ...
    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
      <UCard v-for="stat in stats" :key="stat.title">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="text-sm text-neutral-500 dark:text-neutral-400">{{ stat.title }}</p>
            <p class="text-2xl font-semibold text-neutral-900 dark:text-white">{{ stat.value }}</p>
          </div>
        </div>
      </UCard>
    </div>
    ...
    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      <UCard>
        <template #header>
          <h3 class="font-semibold">快速入口</h3>
        </template>
        <div class="space-y-2">
          <UButton
      ...
      <UCard v-if="!user?.emailVerified">
        <template #header>
          <h3 class="font-semibold text-amber-600">邮箱未验证</h3>
        </template>
        <p class="text-sm text-neutral-600 dark:text-neutral-400 mb-4">
          请验证您的邮箱以获得完整功能访问
        </p>
        <UButton @click="resendVerification" :loading="resending" size="sm">
          重新发送验证邮件
        </UButton>
      </UCard>
    </div>
