...
onMounted(async () => {
  const code = route.query.authCode as string
  ...
  try {
    const result = await post<{ accessToken: string, refreshToken: string }>('/oauth/dingtalk/login', { code })
    ...
    error.value = e.data?.message || e.message || '钉钉登录失败'
    loading.value = false
  }
})
</script>
